...
function getUser() {
  ...
  function initNavbarUser() {
    ...
    // Muestra el dropdown y escribe el username
    const udc = document.getElementById("userDropdownContainer");
    const ud  = document.getElementById("userDropdown");
    if (udc && ud) {
      udc.style.display = "block";
      ud.textContent = user.username;    // ← aquí pones el nombre en el navbar
    }
  
    // Actualiza también el balance
    document.getElementById("totalCredits").textContent = `$${user.balance}`;
  }
  ...
  function updateBalanceUI(betAmount = 0) {
    ...
    if (totalCredits) totalCredits.textContent = `$${user.balance}`;
  
    const betVal = document.getElementById("bet-value");
    if (betVal) betVal.textContent = `$${betAmount}`;
  }
  ...
  // 2. Estilos y tamaño de cartas
  
  (function injectStyles() {
    const style = document.createElement("style");
    style.innerHTML = `
      @keyframes deal { 0%{opacity:0;transform:translateY(-50px) scale(.8);}100%{opacity:1;transform:translateY(0) scale(1);} }
      .deal-card { opacity: 0; animation: deal .4s ease forwards; }
      .card-img, .deal-card { width: 64px !important; height: auto !important; }
      #game-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.7);display:flex;align-items:center;justify-content:center;flex-direction:column;color:#fff;font-size:2rem;opacity:0;visibility:hidden;transition:opacity .3s;z-index:9999;}
  ...
  class Blackjack {
    constructor() {
      const user = getUser();
      this.credits   = user ? user.balance : 1000;
      this.deck      = null;
      this.player    = null;
      this.dealer    = null;
      this.isOver    = false;
    ...
    start(bet) {
      ...
      this.player = new Hand(); this.dealer = new Hand();
      this.isOver = false;
  
      const p0 = this.deck.draw(), d0 = this.deck.draw();
      const p1 = this.deck.draw(), d1 = this.deck.draw();
      this.player.add(p0); this.dealer.add(d0);
      this.player.add(p1); this.dealer.add(d1);
  
      this._clearUI(); this._disableActions();
      [
    ...
    async _finish() {
      ...
      let msg = "", delta = 0;
      if (p > 21)               msg = `¡Te pasaste! Pierdes ${this.bet}`;
      else if (d > 21 || p > d) { msg = `¡Ganaste ${this.bet}`; delta = this.bet*2; }
      else if (p < d)           msg = `Perdiste ${this.bet}`;
      else                      { msg = `Empate. Recuperas ${this.bet}`; delta = this.bet; }
  ...
  function showOverlay(text, onClose) {
    ...
    if (!ov) {
      ov = document.createElement("div"); ov.id = "game-overlay";
      ov.style = "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;flex-direction:column;color:#fff;font-size:2rem;";
      const p = document.createElement("p"); ov.appendChild(p);
      const btn = document.createElement("button"); btn.innerText = "Jugar de nuevo"; btn.style = "margin-top:1rem;padding:0.5rem 1rem;font-size:1rem;"; btn.onclick = () => { hideOverlay(); onClose && onClose(); };
      ov.appendChild(btn);
      document.body.appendChild(ov);
    }
    ov.querySelector("p").innerText = text;
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    if (slider && valEl) {
      ...
      slider.addEventListener("input", () => valEl.innerText = slider.value);
    }
  
    dealBtn?.addEventListener("click", () => {
      const bet = parseInt(valEl.innerText.replace("$",""), 10) || 0;
      game.start(bet);
    });
